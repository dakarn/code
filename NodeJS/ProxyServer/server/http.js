...
function run( req, res )
{
	...
	try
	{
		...
		if( !Opt.checkMaxClient( res, 0 ) ) return;

		var len_data_traffic = 0;
		var index_user = -1;

		if( (index_user = Options.checkAccessHttp( res, req, false )) === false ) return;
		if( !Opt.checkMaxThreadsKey(res, index_user, 0) ) return;

		var ph = url.parse(req.url);
		var IP = req.connection.remoteAddress.split(":").pop();

		if( !Options.deniedIP( IP, res, 'http' ) ) return;
		if( !Options.blackListHost( ph.hostname.toString().trim(), res, 'http' ) ) return;
		...
		proxyRequest.on('response', function(proxyResponse)
		{
			...
			proxyResponse.on('data', function(chunk)
			{
				...
				if( Options.checkLimitTraffic(index_user) === false )
				{
					...
					{
						proxyResponse.pause(); var diff = Opt.limitSpeed() - len_data_traffic;
						setTimeout(function () { len_data_traffic = 0; proxyResponse.resume(); }, 1000+Math.floor(Math.abs(diff)/100));
					}
				}
